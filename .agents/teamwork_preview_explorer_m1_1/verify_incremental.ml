(**
   verify_incremental.ml - Test incremental SHA-256 context implementation
*)
...
module Crypto = struct
  ...
  let finalize_bytes (ctx : ctx) : bytes =
    ...
    if ctx.buf_len > 56 then (
      (* Pad with zeros to 64, process block, then pad next block with zeros up to 56 *)
...
let () =
  ...
  Printf.printf "=== Testing Direct & Incremental Hashing ===\n";
  List.iter (fun (input, expected) ->
    let actual_direct = Crypto.sha256_string input in
    if actual_direct <> expected then failwith (Printf.sprintf "Direct mismatch for '%s'" input);

    (* Test chunked updates of various chunk sizes: 1, 2, 7, 16, 32, 64 *)
    let chunk_sizes = [1; 2; 7; 16; 32; 64] in
    List.iter (fun c_size ->
      let ctx = Crypto.init () in
      let len = String.length input in
      let pos = ref 0 in
      while !pos < len do
        let take = min c_size (len - !pos) in
        Crypto.update_string ctx (String.sub input !pos take);
        pos := !pos + take
      done;
      let actual_chunked = Crypto.finalize_hex ctx in
      if actual_chunked <> expected then
        failwith (Printf.sprintf "Chunked (%d) mismatch for '%s'" c_size input)
    ) chunk_sizes;

    Printf.printf "  [PASS] len %d test vector\n" (String.length input);
  ) test_cases;

  (* Large 1MB test via streaming *)
  let million_a_ctx = Crypto.init () in
  let chunk_10k = String.make 10000 'a' in
  for _ = 1 to 100 do
    Crypto.update_string million_a_ctx chunk_10k
  done;
  let actual_million = Crypto.finalize_hex million_a_ctx in
  ...
  if actual_million = expected_million then
    Printf.printf "  [PASS] 1,000,000 'a' streamed test vector\n"
  else
    failwith "Million 'a' streamed mismatch";
