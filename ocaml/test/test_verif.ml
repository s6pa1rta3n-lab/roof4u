...
let () =
  ...
  let res_inv1_pass = check_physical_eligibility Victorian SingleFamily in
  assert_true "INV-1 passes for Victorian SFR" (match res_inv1_pass with Satisfied _ -> true | _ -> false);

  let res_inv1_pass_flat = check_physical_eligibility Flat MultiUnit2To4 in
  assert_true "INV-1 passes for Flat 2-4 Unit" (match res_inv1_pass_flat with Satisfied _ -> true | _ -> false);

  let res_inv1_fail = check_physical_eligibility Gable SingleFamily in
  assert_true "INV-1 fails for Gable roof" (match res_inv1_fail with Violated _ -> true | _ -> false);

  let res_inv1_fail_comm = check_physical_eligibility Victorian Commercial in
  ...
  let res_inv4_pass = check_permit_recency [permit_old] in
  ...
  let res_inv4_fail = check_permit_recency [permit_recent] in
  ...
  assert_true "Score is bounded <= 100.0" (score_high.total_actionability_score <= 100.0);
  assert_true "Score is bounded >= 0.0" (score_high.total_actionability_score >= 0.0);
  assert_true "Score for prime Victorian is >= 85.0" (score_high.total_actionability_score >= 85.0);
  ...
  assert_true "Score for baseline Flat 2-4 unit is between 50.0 and 80.0"
    (score_low.total_actionability_score >= 50.0 && score_low.total_actionability_score <= 80.0);

  (* 7. Test Lead Verification End-to-End *)
  let qualified_lead_raw = {
    address = "2223 Pacific Ave";
    zip_code = "94115";
    property_type_str = "Single-Family";
    roof_type_str = "Victorian";
  ...
  assert_true "Full lead verification passes for prime lead" (match verified_res.verdict with Qualified _ -> true | _ -> false);

  let json_out = verified_lead_to_json verified_res in
  assert_true "JSON output contains QUALIFIED status" (String.length json_out > 50 && (try let _ = Str.search_forward (Str.regexp_string "QUALIFIED") json_out 0 in true with _ -> false));
